...
const checks = [
  ...
  ['panel izquierdo ocultable', 'id="toggleLeftPanelBtn"'],
  ['zonas fijas del editor', 'grid-template-areas:"left board right"'],
  ['mapa anclado a su zona', '.boardShell{grid-area:board}'],
  ['editor derecho anclado', '#rightPanel{grid-area:right}'],
  ['breakpoint oculto estable', '.workspace.leftCollapsed{grid-template-columns:0 minmax(0,1fr) 350px}'],
  ['cuadrícula ampliada sin brújula', '.workspace.leftCollapsed{grid-template-columns:0 minmax(0,1fr)'],
  ['preferencia del panel recordada', 'LA_RAYUELA_LEFT_PANEL_COLLAPSED_V1'],
